...
# apply a runtime per punch_cost_per_sheet and estimate number of punches to accomplish multi hit features using a factor to runtime
runtime_per_punch = var('Time per punch_cost_per_sheet', 1.5, 'Time per punch_cost_per_sheet in seconds', number)
multi_hit_per_punch_length = var(
    'Multi hit per punch_cost_per_sheet length', 2, 'Amount of cut length removed per stroke for multi-hit contours', number
)
multi_hit_count = var('Multi hit count', 0, 'Total number of hits to punch_cost_per_sheet multi-hit contours', number, frozen=False)
