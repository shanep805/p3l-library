# This operation is for costing a punch laser machine. This operation assumes it will hit all single hits with the punch
...
laser_pierce_count = var('pierce count', 0, 'number of times laser needs to pierce', number, frozen=False)
...
# update laser runtime
laser_runtime = var('Laser Time', 0, 'Minutes of laser time', number, frozen=False)
laser_runtime.update(laser_cut_length / mat_cut_rate + laser_pierce_count * mat_pierce_time / 60)
laser_runtime.freeze()

# now calculate punch setup time based on unique tools needed
...
punch_tool_setup_time = var('Punch Tooling Setup', 0, 'Setup time in minutes for unique punch tools', number, frozen=False)
punch_tool_setup_time.update(setup_time_per_tool * single_hit_setups)
punch_tool_setup_time.freeze()


# now calculate punch runtime based on single hits only and a time per hit
...
# now update total setup time as sum of punch tool and base setup time
...
# now update runtime as a sum of laser and punch runtimes
