...
base_bend_setup_time = var('Base Bend Setup Time', 5, 'Base setup time in minutes for each bend', number)
st_increment = var('Setup Time Increment', 2.5, 'Setup time increment in minutes for each difficulty point', number)

# establish base runtime and time increments in seconds
base_bend_runtime = var('Base Bend Runtime', 30, 'Base runtime in seconds for each bend', number)
rt_increment = var('Runtime Increment', 2.5, 'Runtime increment in seconds for each difficulty point', number)

# establish geometric thresholds for incrementing times
# in inches
medium_radius_thresh = var('Medium Radius Thresh', 2.0, 'Threshold in inches for bend radius being +1 difficulty point', number)
large_radius_thresh = var('Large Radius Thresh', 4.0, 'Threshold in inches for bend radius being +2 difficulty points', number)
medium_length_thresh = var('Medium Length Thresh', 20.0, 'Threshold in inches for bend length being +1 difficulty point', number)
large_length_thresh = var('Medium Length Thresh', 40.0, 'Threshold in inches for bend length being +2 difficulty points', number)
