...
# We will use the values associated with this interrogation in subsequent variables.
...
# We omit 'hole' to as to not double count features
...
features.freeze()
# We set the Setup Time to a minimum of 0.5 hours (30 minutes).
# If the calculated programming time is greater than 30 minutes, we use that time.
setup_time.update(max(0.5, (features * time_per_feature) / 60))
setup_time.freeze()
